function [P1,b_evt_data,O_evt_data,beta,temp_b,sum_b_o] = fwkde2G(space_time_data,V_b,Xs_b,Ys_b,Tt_b,Ds_b,Dt_b,V_o,X_o,Y_o,T_o,D_o,n,n1,x1,y1,t1,Temps_b,Tempt_b,Temp_o)
...
%Here we only examine the general situation of inseparabili
% ty
...
for j = 2:N
    ...
    if Uj < P1(j,1)
        ...
        G0 = P1(j,1);
        for i = 2:(j-1)
            G0 = G0 + P0(j-1,i-1);  % Note: j-1 and i-1 because P0 = P1(2:end,2:end)
            if Uj < G0
                G(j) = i;
                break;
            end
...
% Immigrant events (background): spatial and temporal coordinates
b_evt_data = event_data(b_evt, :);  % [x, y, t] for immigrant events
